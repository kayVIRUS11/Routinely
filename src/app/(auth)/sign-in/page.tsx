  ...
  return (
    <div className="w-full max-w-sm">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        {error && (
          <div className="flex items-start gap-2 text-sm text-error bg-error/10 px-3 py-2 rounded-lg">
            <AlertCircle className="w-4 h-4 mt-0.5 shrink-0" />
            <span>{cooldownEnd ? `Too many attempts. Retry in ${remaining}s.` : error}</span>
          </div>
        )}

        <Button type="submit" loading={loading} disabled={!!cooldownEnd} className="w-full mt-2">
          Sign in
        </Button>
